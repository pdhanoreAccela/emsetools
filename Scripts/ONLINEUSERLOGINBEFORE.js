/*------------------------------------------------------------------------------------------------------/
| SVN $Id: OnlineUserLoginBefore.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : OnlineUserLoginBeforeV1.5.js
| Event   : OnlineUserLoginBefore
|
...
var controlString = "OnlineUserLoginBefore"; // Standard choice for control
...
logDebug("<B>EMSE Script Results for online user = " + username + "</B>");

var publicUserModelResult = aa.publicUser.getPublicUserByUserId(username)
	{
	publicUserModel = publicUserModelResult.getOutput();
	if (publicUserModel)
		{
		...
		logDebug("publicUserModel = " + publicUserModel.getClass());
...
if (contractorLicenseList != "" && contractorLicenseList != null)
	...
	for (var i=0; i< contractorLicenseList.size(); i++)
		...
		logDebug("lModel = " + lModel);
		logDebug("licenseNbr = " + licenseNbr + "  licenseType = " + licenseType);
...
if (debug.indexOf("**ERROR") > 0)
	...
	aa.env.setValue("ReturnMessage", debug);
	}
else
	{
	if (cancel)
		{
		aa.env.setValue("ReturnCode", "-1");
		if (showMessage) aa.env.setValue("ReturnMessage", message);
		if (showDebug) 	aa.env.setValue("ReturnMessage", debug);
		}
	else
		{
		aa.env.setValue("ScriptReturnCode", "0");
...
//
	...
	if (getCapResult.getSuccess())
		...
		if (linkResult.getSuccess())
			...
			logDebug( "**ERROR: linking to parent application parent cap id (" + parentAppNum + "): " + linkResult.getErrorMessageh;i++)
...
function autoAssignInspection(iNumber)
	...
	inspectorObj = inspectorArr[0] just document
			{
			docWrite("|  ",false,docIndent);
			var disableString = "";
			if (!doObj.enabled) disableString = "<DISABLED>";
...
//
	...
	logDebug("creating ca just document
			{
			docWrite("|  ",false,docIndent);
			var disableString = "";
			if (!doObj.enabled) disableString = "<DISABLED>";
...
//
	...
	if (getCapResult.getSuccess())
		...
		if (linkResult.getSuccess())
			...
			logDebug( "**ERROR: linking to parent application parent cap id (" + parentAppNum + "): " + linkResult.getErrorMessageibuteName.equals(peopAttrArray[i].getAttributeName()))
			{
...
//
	...
	if (getCapResult.getSuccess())
		...
		if (linkResult.getSuccess())
			...
			logDebug( "**ERROR: linking to parent application parent cap id (" + parentAppNum + "): " + linkResult.getErrorMessages the capId object of the parent.  Assumes only one parent!
	//
...
//
	...
	if (getCapResult.getSuccess())
		...
		if (linkResult.getSuccess())
			...
			logDebug( "**ERROR: linking to parent application parent cap id (" + parentAppNum + "): " + linkResult.getErrorMessageetDispositionNote();
...
function proximity(svc,layer,numDistance)  // optional: distanceType
	...
	if (bufferTargetResult.getSuccess())
		...
		{ logDebug("**WARNING: Getting GIS Type for Buffer Target.  Reason is: " + bufferTargetResult.getErrorType() + ":" + bufferTargetResult.getErrorMessage()) ; ret just document
			{
			docWrite("|  ",false,docIndent);
			var disableString = "";
			if (!doObj.enabled) disableString = "<DISABLED>";
...
//
	...
	if (getCapResult.getSuccess())
		...
		if (linkResult.getSuccess())
			...
			logDebug( "**ERROR: linking to parent application parent cap id (" + parentAppNum + "): " + linkResult.getErrorMessageug("dollarAmount parameter is not a number, no funds will be transferred");
		return false;
		}
