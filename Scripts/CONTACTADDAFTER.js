/*------------------------------------------------------------------------------------------------------/
| SVN $Id: ContactAddAfter.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : ContactAddAfterV1.5.js
...
var sca = String(aa.env.getValue("CapId")).split("-");
var capId = aa.cap.getCapID(sca[0],sca[1],sca[2]).getOutput()

if (!capId) capId = getCapId();							// CapId object
...
var contactFirstName = ContactObj.getPeople().getFirstName() ;	logDebug("contactFirstName : " + contactFirstName);
var contactLastName = ContactObj.getPeople().getLastName() ;	logDebug("contactLastName : " + contactLastName);
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	{
	// Optional 6th argument i&& (pDesc==null || pDesc.toUpperCase()==cDesc.toUpperCase()) && (pImpact==null || pImpact.toUpperCase()==cImpact.toUpperCase()) )
								{
...
function appMatch(ats) // optional capId or CapID string
	...
	if (arguments.length == 2)
		...
		if (!matchCapId)
			{
			logDebug("**WARNING: CapId passed: " + contactLastName);
var contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
/*------------------------------------------------------------------------------------------------------/
| END Event Specific Variables
/------------------------------------------------------------------------------------------------------*/
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	{
	// Optional 6th argument i& (!useAppSpecificGroupName || appspecObj[i].getCheckboxType() == itemGroup))
					{
...
function getRefLicenseProf(refstlic)
	...
	if (!refLicenseResult.getSuccess())
		{ logDebug("**ERROR retrieving Ref Lic Profs : " + refLicenseR: " + contactLastName);
var contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
/*------------------------------------------------------------------------------------------------------/
| END Event Specific Variables
/------------------------------------------------------------------------------------------------------*/
...
function lookupDateRange(stdChoiceEntry,dateValue) // optional val number
	...
	var compDate = new Date(: " + contactLastName);
var contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
/*------------------------------------------------------------------------------------------------------/
| END Event Specific Variables
/------------------------------------------------------------------------------------------------------*/
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	{
	// Optional 6th argument ilength != 4)
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice,pDuplicate,pFeeSeq)
    ...
    // Optional param pSeqNumber, Will attempt to update the specified Fee Sequence Number or: " + contactLastName);
var contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
/*------------------------------------------------------------------------------------------------------/
| END Event Specific Variables
/------------------------------------------------------------------------------------------------------*/
