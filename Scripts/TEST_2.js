...
function loadAppSpecific(thisArr) {
	...
	//
	var appSpecificGroups = cap.getAppSpecificInfoGroups().toArray();
	
	for (xx in appSpecificGroups)
	{
		var appSpecificGroup = appSpecificGroups[xx];		
		var fields = appSpecificGroup.getFields().toArray();
		...
		for (yy in fields)
		{
			...
			if (useAppSpecificGroupName)
				thisArr[field.getCheckboxType() + "." + field.getFieldLabel()] = field.getChecklistComment();
			else
				thisArr[field.getFieldLabel()] = field.getChecklistComment();
		}
	}
}
...
function loadASITables() {
	...
	while (tai.hasNext())
	  ...
	  var tsmcoli = tsm.getColumns().iterator();
	  var numrows = 1;
// Added RN 10/19/10 - 2 lines
//if (tn != 'TRADENAMESOPERATINGNAME')
//{
	  while (tsmfldi.hasNext())  // cycle through fields
		{
		if (!tsmcoli.hasNext())  // cycle through columns
...
function refLicProfGetDate (pLicNum, pDateType)
	...
	//if (!addResult .getSuccess())
	//	{ aa.print("**WARNING: error removing all rows from ASI Table:  " + tableName + " " + addResult.getErrorMessage()) ; return false }
	//else
...
function getAppSpecForACA(itemName) {
	...
	for (xx in appSpecificGroups) {
		...
		for (yy in fields) {
			var field = fields[yy];
			if (field.getFieldLabel().equals(itemName)) {
