...
| Action# : N/A
|
| Notes   :Added function getParentCapIDForReview for SAC County
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
function externalLP_CA(licNum,rlpType,doPopulateRef,doPopulateTrx,itemCap)
	...
	*/
	...
	for (var thisLic = 0; thisLic < workArray.length; thisLic++)
        ...
        aURLArgList[0] = "https://www.achievo.com:8080/IVR/License+Detail.asp?LicNum=" + licNum;
		...
		if (doPopulateRef)  // refresh or create a reference LP
			...
			if (lpBiz.getChild("BusinessPhoneN.getText() != "") newLic.setPhone1(unescape(stripNN(lpBiz.getChild("BusinessPhoneNum").getText()).replace(/\+/g," ")));
			newLic.setAgencyCode(aa.getServiceProviderCode());
			newLic.setAuditDate(sysDate);
			newLic.setAuditID(currentUserID);
			newLic.setAuditStatus("A");
...
function getCSLBInfo(doPop,doWarning)   // doPop = true populate the cap lic prof with this data  
	...
	var getout = aa.util.httpPost("http://www.achievo.com:8080/VR/License+Detail.asp?LicNum=" + rlpId,"");
...
function getGISInfoArray(svc,layer,attributename)
	...
	var bufferTargetResult = aa.gis.getGISType(svc,layer); // get the bufftarget
	if (bufferTargetResult.getSuccess())
		{
		var buf = bufferTargetResult.getOutput();
		buf.addAttributeName(attributename);
