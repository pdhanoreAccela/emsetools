...
logMessage("currentUserID1 log: " + aa.env.getValue("CurrentUserId"));
logMessage("CurrentUserID2 log: " + aa.env.getValue("CurrentUserID"));
		...
		for(var i = 0; i < recordTypeList.size(); i++)
		{
			...
			else
			{
				logError("**ERROR copy guideSheet items to cap: " + copyResult.getErrorMessage());
                                logError("currentUserID1 error: " + aa.env.getValue("CurrentUserId"));
                                logError("CurrentUserID2 error: " + aa.env.getValue("CurrentUserID"));
			}	
			logMessage("*********************************************************************************");
		}
	}
