/*------------------------------------------------------------------------------------------------------/
| Program : ACA_Amend_ASI_Before_V1.0.js
| Event   : ACA_Amend_ASI_Before
|
...
var controlString = "ACA Amend PageFlowScript Before ASI";		// Standard choice for control
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(Status,pDesc,pImpact)
	{
...
function branchTask(wfstr,wfstat,wfcomment,wfnote) // optional process name
	{
	var useProcess = false;
	var p formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(tions from pFromCapId to pToCapId
...
function createRefLicProf(rlpId,rlpType,pContactType)
	...
	newLic.setAud formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(te.getMonth()+1).toString() +"/"+jsStartDate.getDate()+"/"+jsStartDate.getFullYear() + " End Date: " + (jsEndDate.getMonth()+1).toString() +"/"+jsEndDate.getDate()+"/"+jsEndDate.getFullYear());
...
function getChildren(pCapType, pParentCapId)
	...
	for (xx in childArray)
		...
		if (isMatch)
			retA formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(xpDate(expAADate);
...
function loadTaskSpecific(thisArr)
	{
 	...
 	// If useTaskSpecific formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(.handleDisposition(capId,stepnumber,pStatus,dispositionDate,wfnote,pComment,systemUserObj,"Y");
			...
			if (assignDate)
				...
				var adjustResult = aa.workflow.adjustTaskWithNoAudit(taskItem);
              				...
              				logDebug("Updated Workflow Task : " + wfstr + " Assigned Date to " + wfAssignDate);
                                      ...
                                      formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(