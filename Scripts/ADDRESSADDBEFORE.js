/*------------------------------------------------------------------------------------------------------/
| Program : AddressAddBeforeV1.5.js
...
var controlString = "AddressAddBefore";	 				// Standard choice for control
...
//if parcelNum is null, condition is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			{
			vdition.addAddressCondition(pAddrNum, pType, pDesc, pComment, null, null, pImpact, pStatus, sysDate, null, sysDate, sysDate, systemUserObj, systemUserObj); 
...
function assignInspection(iNumber,iName)
	...
	if (!iObjResult.getSuccess())
		{ logDebug("**ERROR retrieving insp("ScriptReturnCode", "1");
		if (showMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
...
//if parcelNum is null, condition is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			{
			vlResult = aa.finance.getCalculatedValuation(fromcap,null);
...
function editTaskSpecific(wfName,itemName,itemValue)  // optional: itemCap
	...
	//
 	...
 	//
 	for (i in wfOb("ScriptReturnCode", "1");
		if (showMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
...
//
	...
	if (retArr.length > 0)("ScriptReturnCode", "1");
		if (showMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			logDebug("Closing Workflow Task: " + wfstr + " with status " + wfstat + ", Loopin("ScriptReturnCode", "1");
		if (showMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
...
//if parcelNum is null, condition is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			{
			vispositionDate = aa.date.getCurrentDate();
	var wfnote = " ";
	var wftask;
...
function updateShortNotes(newSN) // option CapId
	...
	var cdScriptObjResult = aa.cap.getCapDetail(itemCap);
	if (!cdScriptObjResult.getSuccess())
		{ logDebug("**ERROR: No cap detail script object : " + cdScriptObj("ScriptReturnCode", "1");
		if (showMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
