/*------------------------------------------------------------------------------------------------------/
| Program : ApplicationConditionAddAfterV1.5.js
...
var controlString = "ApplicationCondition" 			// Standard choice for control
...
function assignInspection(iNumber,iName)
	{
	// updates the inspection andnvoicing of fees
//
if (feeSeqList.length)
	{
	invoiceResult = aa.finance.createInvoice(capId, feeSeqList, paymentPeriodList);
...
function addParcelCondition(parcelNum, cType,cStatus,cDesc,cComment,cImpact)
//i8 JHS  creatBCalcValuatn method began using the script model after 6.4  updated this function
	if (!newcap)
...
function copySchedInspections(pFromCapId, pToCapId)
	...
	if (!inspResultObj.genvoicing of fees
//
if (feeSeqList.length)
	{
	invoiceResult = aa.finance.createInvoice(capId, feeSeqList, paymentPeriodList);
...
function addParcelCondition(parcelNum, cType,cStatus,cDesc,cComment,cImpact)
//ioxObj = proxArr[a2].getGISObjects();  // if there are GIS Objects here, we're done
			for (z1 in proxObj)
				{
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			if (useProcess)
				...
				aa.workflow.handleDisposition(capId,stepnumber,wfstat,dispositionDate, wfnote,wfcommentnvoicing of fees
//
if (feeSeqList.length)
	{
	invoiceResult = aa.finance.createInvoice(capId, feeSeqList, paymentPeriodList);
...
function updateShortNotes(newSN) // option CapId
	{
	var itemCap = capId
	if (arguments.length > 1) itemCap = argumnvoicing of fees
//
if (feeSeqList.length)
	{
	invoiceResult = aa.finance.createInvoice(capId, feeSeqList, paymentPeriodList);
