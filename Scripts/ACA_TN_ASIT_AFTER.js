/*------------------------------------------------------------------------------------------------------/
| Program : ACA_TN_ASIT_After_V1.0.js
| Event   : ACA_TN_ASIT_After
|
...
var controlString = "ACA TN PageFlowScript After ASIT"; 			// Standard choice for control
...
function autoAssignInspection(iNumber)
	...
	if (!assignResult.getSuccess())
		{ logDebug("**ERROR re-assigning inspection " + assignRes------------------------*/
function dateFormatted(pMonth,pDay,pYear,pFormat)
//returns date string formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
...
Debug("Copied parcel "+Parcels[zz].getParcelNumber()+" from "+pFromCapId.getCustomID()+" to "+vToCapId.getCustomID());
			copied++;
			}
		}
	else
...
ets total for fees assessed during date range
...
function getChildren(pCapType, pParentCapId)
	...
	for (xx in childArray)
		...
		capTypeStr = aa.cap.getCap(childCapId).getOut------------------------*/
function dateFormatted(pMonth,pDay,pYear,pFormat)
//returns date string formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
...
function loadASITable(tname) {
	...
	while (tai.hasNext())
      ...
      var readOnlyi = tsm.getReadonlyField().iterator(); // get Readonly filed
	  var numrows = 1;
...
function loadASITables() {
	...
	while (tai.hasNext())
	  ...
	  var readOnlyi = tsm.getReadonlyField().iterator();
...
leInspection(capId, inspectorObj, aa.date.parseDate(dateAdd(null,DaysAhead)), null, iType, "Scheduled via Script")
...
function updateShortNotes(newSN) // option CapId
	...
	for (i in wfObj)
		...
		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			var processID = fTask.getProcessID();
			if (useProcess)
				aa.workflow.handleDisposition(itemCap,stepnumber,processID,wfstat,dispositi------------------------*/
function dateFormatted(pMonth,pDay,pYear,pFormat)
//returns date string formatted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
