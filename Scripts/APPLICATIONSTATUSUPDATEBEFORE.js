/*------------------------------------------------------------------------------------------------------/
| SVN $Id: APPLICATIONSTATUSUPDATEBEFORE.js 1249 2008-01-10 18:57:01Z john.schomp $
| Program : APPLICATIONSTATUSUPDATEBEFOREV1.4.js
...
loadASITables();
var newStatus = aa.env.getValue("ApplicationStatus");
...
function addParcelDistrict(parcelme()].readOnly);
...
function autoAssignInspection(iNumber)
	...
	inspSchedDate = iObj.getSchedule
	}
else
	{
	if (cancel)
...
function addParcelDistrict(parcel copyParcels(pFromCapId, pToCapId)
	{
...
function addParcelDistrict(parceln amtFee - amtPaid;
	}
...
function addParcelDistrict(parcelxpiration.getLicensesByCapID(capId,licenseStatus[myStatus]);
...
function addParcelDistrict(parcelsultResult.getErrorMessage())
		}
	else
...
function updateShortNotes(newSN) // option CapId
	...
	if (cdWrite.getSuccess())
		...
		{ logDebug("**ERROR writing capdetail : " + cdWrite.g
	}
else
	{
	if (cancel)
...
function addParcelDistrict(parcelurn licCapArray[aps];
			}
		}
	}
