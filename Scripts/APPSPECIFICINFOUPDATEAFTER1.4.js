/*------------------------------------------------------------------------------------------------------/
| SVN $Id: AppSpecificInfoUpdateAfter.js 1249 2008-01-10 18:57:01Z john.schomp $
| Program : AppSpecificInfoUpdateAfterV1.4.js
| Event   : AppSpecificInfoUpdateAfter
|
...
function autoAssignInspection(iNumber)
	...
	if (inspectorArr == null || inspemessage);
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				{
				apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capId.getID3(),Parcels[zz].getParcelNumber(),districtValue);atus || exists(feeObjArr[ff].getFeeitemStatus(),statusArray) ) )
			return true;
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				{
				apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capId.getID3(),Parcels[zz].getParcelNumber(),districtValue); B1Expiration Object for Cap.  Reason is: " + b1ExpResult.getErrorType() + ":" + gisObjResult.getErrorMessage()) ; return false }
		}
...
function loadParcelAttributes(thisArr) {
	...
	var fcapParcelObj = null;
  	...
  	for (i in fcapParcelObj)
		...
		thisArr["ParcelAttribute.Lamessage);
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				{
				apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capId.getID3(),Parcels[zz].getParcelNumber(),districtValue);cheduled inspection : " + iType + " for " + DateToSched);
	else
...
function updateShortNotes(newSN) // option CapId
	...
	{ logMessage("**ERROR: Failed to get workflow object: " +message);
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	cd.setEstimatedDueDate(aa.date.parseDate(receipttodate));
...
function createReferenceLP(rlpId, rlpType, pContactType, bizname1, bizname2) {
    ...
    if (myResult.getSuccess()) {
        ...
        lpsm = aa.licenseScript.getRefLicenseProfBySeqNbr(servProvCode, myResult.getOutput()).getOutput();
        aa.licenseScript.associateLpWithCap(capId, lpsm)
