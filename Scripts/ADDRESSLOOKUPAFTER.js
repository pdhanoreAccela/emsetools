/*------------------------------------------------------------------------------------------------------/
| Program : AddressLookUpAfterV1.5.js
...
capString= aa.env.getValue("CapIdModel")
capId = aa.cap.getCapID(capString).getOutput() 
...
function assignTask(wfstr,username) // optional process name
	...
	if (arguments.length == 3)
		{
		processName = arguments[2]; // subpbug);
	}
else
	{
	aa.env.setValue("ScriptReturnCode", "0");
			...
			for (zz in Parcels)
				...
				var addParcelCondResult = aa.parcelCondition.addParcelCondition(Parcels[zz].getParcelNumber(), cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate, systemUserObj, systemUserObj);
					if (addParcelCondResult.getSuccess())
					        	{
						logMessage("Successfully added condition to Parcel " + Parcels[zz].getParcelNumber() + "  (" + cImpact + ") " + cDesc);
						logDebug("Successtail : " + cdWrite.getErrorMessage()) ; return false ; }
	}
 			...
 			if (TSIResult.getSuccess())
				...
				if (TSI != null)
					...
					if (TSIUResult.getSuccess())
						...
						{ logDebug("**ERROR: Failed to Update Task Specific Info : " + TSIUResult.getErrorMessage()); bug);
	}
else
	{
	aa.env.setValue("ScriptReturnCode", "0");
			...
			for (zz in Parcels)
				...
				var addParcelCondResult = aa.parcelCondition.addParcelCondition(Parcels[zz].getParcelNumber(), cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate, systemUserObj, systemUserObj);
					if (addParcelCondResult.getSuccess())
					        	{
						logMessage("Successfully added condition to Parcel " + Parcels[zz].getParcelNumber() + "  (" + cImpact + ") " + cDesc);
						logDebug("SuccessroxArr = bufchk.getOutput();
		else
			...
			for (zz in Parcels)
				...
				var addParcelCondResult = aa.parcelCondition.addParcelCondition(Parcels[zz].getParcelNumber(), cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate, systemUserObj, systemUserObj);
					if (addParcelCondResult.getSuccess())
					        	{
						logMessage("Successfully added condition to Parcel " + Parcels[zz].getParcelNumber() + "  (" + cImpact + ") " + cDesc);
						logDebug("SuccesspParcelObj[i].getMapRef();
...
function updateTask(wfstr,wfstat,wfcomment,wfnote) // optional process name, cap id
	...
	for (i in wfObj)
		...
		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			var processID = fTask.getProcessID();
			if (useProcess)
				aa.workflow.handleDisposition(itemCap,stepnumber,processID,wfstat,dibug);
	}
else
	{
	aa.env.setValue("ScriptReturnCode", "0");
