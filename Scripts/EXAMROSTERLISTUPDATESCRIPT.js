...
function getParamsForUnschedule(rosterModel, capID)
{
	...
	setExamNameAndProviderName(rosterModel);
...
{	
	...
	{		
		setExamNameAndProviderName(oldRosterModel);
		var examName =  aa.env.getValue("examName");
		var providerName = aa.env.getValue("providerName");
...
function getExamSite(exam)
{
	...
	if(examSite.getSuccess() && examSite.getOutput() != null)
	{
		...
		var bizDomain = aa.bizDomain.getBizDomainByValueWithSD("COUNTRY",location.getCountryCode());
		if(bizDomain.getSuccess() && bizDomain.getOutput() != null)
		{
			country = bizDomain.getOutput().getDescription();
			site = appendAddress(site, country);
		}
...
function setExamNameAndProviderName(rosterModel)
{
	var examMapResult = aa.examination.getXRefExaminationProviderModelByRoster(rosterModel);	
	if(examMapResult.getSuccess())
	{
	   var refExamProviderModel = examMapResult.getOutput();
	   providerName = refExamProviderModel.getProviderModel().getProviderName();
	   examName = refExamProviderModel.getRefExaminationModel().getExamName();
