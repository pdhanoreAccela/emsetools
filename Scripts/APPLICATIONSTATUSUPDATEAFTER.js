	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if allTasksComplete(stask) // optional tasks to ignore... for Sacramento
	{
...
function autoAssignInspection(iNumber)
	...
	if (inspectorArr == null || inspectorArr.le
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if 		var newCapParcel = aa.parcel.getCapParcelModel().getOutput();
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if OR: getting fee items: " + capContResult.getErrorMessage()); return false }
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if g("Found renewal record of status : " + this.b1Status + ", Expires on " + this.b1ExpDate);
    				break
    				}
			}
		else
...
function loadParcelAttributes(thisArr) {
	...
	var fcapParcelObj = null;
  	...
  	for (i in fcapParcelObj)
		...
		thisArr["ParcelAttribute.ImprovedValue"]
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
	...
	if (!parcelNum)
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if duleInspection(capId, inspectorObj, aa.date.parseDate(DateToSched), null, iType, "Scheduled via Script")
...
function updateShortNotes(newSN) // option CapId
	...
	var workflowResult = aa.workflow.
	if (showDebug) 	aa.env.setValue("ScriptReturnMessage", debug);
	}
