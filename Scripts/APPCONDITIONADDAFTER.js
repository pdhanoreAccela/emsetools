...
var conditionIds = aa.env.getValue("ConditionId");				logDebug("conditionId = " + conditionIds);
...
// If you add two or more conditions, it will return a string condition-ids like '8641|8642|8143'. So we need to split it with '|';
var conditionIds = conditionIds.toString();
if (conditionIds.indexOf("\\|") != -1)
{	
	var conditionIDArray = conditionIds.split("\\|");
	for (var i = 0; i < conditionIDArray.length; i++)
	{
		var conID = parseInt(conditionIDArray[i]);
		var conditionObj = aa.capCondition.getCapCondition(capId, conID).getOutput();
		//logDebug("conditionObj = " + conditionObj.getClass());
	}
}
else
{
	var conditionObj = aa.capCondition.getCapCondition(capId, conditionIds).getOutput();
	//logDebug("conditionObj = " + conditionObj.getClass());
}
