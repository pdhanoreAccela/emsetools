/*------------------------------------------------------------------------------------------------------/
| Program : ACA_TNB_TN_Before_V1.0.js
| Event   : ACA_TNB_TN_Before
|
...
var controlString = "ACA TNB PageFlowScript Before TN";			// Standard choice for control
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capI,pDesc,pImpact)
	{
...
function branchTask(wfstr,wfstat,wfcomment,wfnote) // optional process name
	{
	var useProcess = false;
	var processtted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capIfrom pFromCapId to pToCapId
...
function createRefLicProf(rlpId,rlpType,pContactType)
	...
	newLic.setAuditDatetted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capIMonth()+1).toString() +"/"+jsStartDate.getDate()+"/"+jsStartDate.getFullYear() + " End Date: " + (jsEndDate.getMonth()+1).toString() +"/"+jsEndDate.getDate()+"/"+jsEndDate.getFullYear());
...
function getChildren(pCapType, pParentCapId)
	...
	for (xx in childArray)
		...
		if (isMatch)
			retArray.ptted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capI(expAADate);
...
function loadTaskSpecific(thisArr)
	{
 	...
 	// If useTaskSpecificGroupName==true, appends wf procestted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capIr,pStatus,dispositionDate,wfnote,pComment,systemUserObj,"Y");
			...
			if (assignDate)
				...
				var adjustResult = aa.workflow.adjustTaskWithNoAudit(taskItem);
              				...
              				logDebug("Updated Workflow Task : " + wfstr + " Assigned Date to " + wfAssignDate);
                                        ...
                                        logDebug("Error updating wfTasktted as YYYY-MM-DD or MM/DD/YYYY (default)
	{
	var mth = "";
	var day = "";
	var ret = "";
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		{
		apdResult = aa.parcel.addParceDistrictForDaily(capId.getID1(),capId.getID2(),capI