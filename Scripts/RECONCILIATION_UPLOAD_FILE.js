// This property is just for test. Please remove it in prodcut enviorment.
var TEST_MODE = true;
...
/**
* Main function for uploading transaction files
**/
function main()
{
	aa.log("start to upload transaction file: ");
	var scriptResult = aa.reconciliationScript.uploadTransactionFile();
	aa.log("Result of uploading transaction file: " + scriptResult.getOutput());

	if (sendMail(scriptResult.getOutput()) && scriptResult.getSuccess())
	{
		aa.env.setValue("ScriptReturnCode", "0");
		aa.env.setValue("ScriptReturnMessage", "Success to upload transaction file.");	
	}
	else
	{
		aa.env.setValue("ScriptReturnCode", "-1");
		aa.env.setValue("ScriptReturnMessage", "Fail to upload transaction file.");	
	}
}

/**
* Initial properties for uploading transaction files
**/
function initialProperties()
{
	...
	aa.reconciliationScript.setTestMode(TEST_MODE);
}
...
{         
	...
	var mailcontent = aa.reconciliationScript.getCustomContentByType(mailContentType, pamaremeters);
...
function getEmailSubject(servProvCode,mailContentType)
{
	...
	addParameter(pamaremeters, "$$servProvCode$$", servProvCode);
	addParameter(pamaremeters, "$$Date$$", currentDate);
	var mailsubject = aa.reconciliationScript.getCustomContentByType(mailContentType, pamaremeters);
	return mailsubject;
}
