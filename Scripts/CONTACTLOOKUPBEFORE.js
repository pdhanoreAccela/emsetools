/*------------------------------------------------------------------------------------------------------/
| SVN $Id: ContactLookupBefore.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : ContactLookupBeforeV1.5.js
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	refLicArr = new Array();
	if (argum.toUpperCase()==cImpact.toUpperCase()) )
								{
...
function appMatch(ats) // optional capId or CapID string
	...
	if (arguments.length == 2)
		...
		matchCap = aa.caeModel.getAttributes();
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	// Optional 6th argument is license number, otherwise add to all CAEs on CAP
	refLicArr = new Array();
	if (argum" + capAddressResult.getErrorMessage());
		return false;
		}
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	// Optional 6th argument is license number, otherwise add to all CAEs on CAP
	refLicArr = new Array();
	if (argum				appspecObj[i].setChecklistComment(itemValue);
...
function editTaskComment(wfstr,wfcomment) // optional process name
	...
	for (i in wfObj)
		{
   		fTask = wfObj[i];
  		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCaseeModel.getAttributes();
...
function getRefLicenseProf(refstlic)
	...
	if (!refLicenseResult.getSuccess())
		...
		{
		var newLicArray = refLicenseResueModel.getAttributes();
...
function lookupDateRange(stdChoiceEntry,dateValue) // optional val number
	...
	var domArr
	for (var count=1; count <= 9999; count++)  // Must be sequentieModel.getAttributes();
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	// Optional 6th argument is license number, otherwise add to all CAEs on CAP
	refLicArr = new Array();
	if (argumg cap type parameter is incorrectly formatted: " + pSearchType);
	}
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice,pDuplicate,pFeeSeq)
    ...
    // If optional argument is blank, use default logic (i.e. allow eModel.getAttributes();
