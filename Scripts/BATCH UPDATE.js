/*------------------------------------------------------------------------------------------------------/
| Program: Batch Voids.js  Trigger: Batch
| Client: ADBCE
|
| Version 1.0 - Base Version. 01/11/10 JHS
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
var daysToExpire = aa.env.getValue("daysToExpire");     // number of work days without payment to void the cap
...
function mainProcess()
	...
	var capFilterValidated = 0;
	var capFilterNotEnoughTimeElapsed = 0;
	var capCount = 0;
	var inspDate;
	var setName;
	var setDescription;
	...
	expResult = aa.cap.getCapListByCollection(null, null, null, aa.date.parseDate(fromDate), aa.date.parseDate(toDate), null, emptyGISArray);
	...
	if (expResult.getSuccess())
		...
		{ logDebug("ERROR: Getting Caps, reason is: " + expResult.getErrorType() + ":" + expResult.getErrorMessage()) ; return false }

	for (thisExp in myExp)  // for each CAP (effectively, each license app)
		{
		...
		var taskArr = loadTasks(capId);
		var taskVVA = taskArr["Verify and Validate Application"];
		if (!taskVVA || (taskVVA && taskVVA.status != "Valid" ))
			{
			capFilterValidated++;
			logDebug(altId + ": skipping due to not being validated yet : ")
			continue;
			}

		var validatedDate = taskVVA.statusdate;
		
		var checkDate = validatedDate;
		for (var i = 0; i< daysToExpire ; i++)
			checkDate = nextWorkDay(checkDate);
			
		checkDate = new Date(checkDate); checkDate.setHours(0); checkDate.setMinutes(0); checkDate.setSeconds(0); checkDate.setMilliseconds(0);

		tDate = new Date() ; tDate.setHours(0); tDate.setMinutes(0); tDate.setSeconds(0); tDate.setMilliseconds(0);
		
		if (tDate <= checkDate)
			{
			capFilterNotEnoughTimeElapsed++;
			logDebug(altId + ": not enough time elapsed yet.  will expire on " + checkDate);
			continue;
			}
	...
	// Create Set
 	...
 	logDebug("Ignored due to not being Validated: " + capFilterValidated);
 	logDebug("Ignored due to not enought time elapsed since validation: " + capFilterNotEnoughTimeElapsed);
...
function scheduleInspectDate(iType,DateToSched) // optional inspector ID.  This function requires dateAdd func.phone2;
