...
function getPeople(capId)
{
	...
	if(s_result.getSuccess())
	{
		...
		if(capPeopleArr != null || capPeopleArr.length > 0)
		{
			...
			{
				var capContactModel = capPeopleArr[loopk];
        var peopleModel = capContactModel.getPeople();
        var contactNbr = peopleModel.getContactSeqNumber();
        var result = aa.address.getContactAddressModel(capId,contactNbr);
        if (result.getSuccess())
        {
        	var contactAddressModel = result.getOutput();
          var contactAddressrs = aa.address.getContactAddressList(contactAddressModel);
