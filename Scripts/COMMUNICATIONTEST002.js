...
associate();
listAvailableEnvs();
sendSMSByModel('Mimic Title', 'Mimic Content')
...
function sendEmailByModel(pTitle, pContent){
	...
	function formContacts(){
		...
		var params = [
						['Tom Liang', 'cindy.qin@achievo.com', to], 
...
function sendSMSByModel(pTitle, pContent){
	...
	function formContacts(){
		...
		var params = [
						['Austin wang', '13681281429', to],
						['Tom Liang', '18675528275', from]
					];
...
function associate(){
	...
	var content = '[329:<13CAP-00000210,RECORD;>]'; 
	var rcmId = /^\s*\[\s*(\d{1,})\s*:/;
	var rEntity = /\s*<(.*)>\s*/g;
...
aa.env.setValue("ScriptReturnCode","0");
aa.env.setValue("ScriptReturnMessage", "COMMUNICATION End");
