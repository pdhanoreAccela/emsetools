/*------------------------------------------------------------------------------------------------------/
| SVN $Id: ContactRemoveAfter.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : ContactRemoveAfterV1.5.js
...
function editTaskSpecific(wfName,itemName,ite--------*/
//
//
//  Get the Standard choices entry we'll use for this App type
//  Then, get the action/criteria pairs for this app
...
function addLookup(stdChoice,stdValue,stdDesc)
	...
	if (stdChoice != null && stdChoice.length && stdValue != null && stdValue.length && stdDesc != null && stdDesc.length)
		{chk.getErrorMessage()) ; return false }
	...
	if (getFeeResult.getSuccess())
		...
		for (feeNum in feeList)
			if (feeList[feeNum].getFeeitemStatus().equals("NEW") && !feeUpdated)  // update this fee item
				...
				if (editResult.getSuccess())
					{
					logDebug("Updated Qty on Existing Fee Item: " + fcode --------*/
//
//
//  Get the Standard choices entry we'll use for this App type
//  Then, get the action/criteria pairs for this app
