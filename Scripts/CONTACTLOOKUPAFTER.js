/*------------------------------------------------------------------------------------------------------/
| SVN $Id: ContactLookupAfter.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : ContactLookupAfterV1.5.js
| Event   : ContactLookupAfter
|
...
var sca = String(aa.env.getValue("CapIDModel")).split("-");
...
var contactList = aa.env.getValue("SelectedContactList");
...
{ 	
	...
	while(its.hasNext())
		{
		ContactObj = its.next();
		peopleModel = ContactObj.getPeople();

	    logDebug("ContactObj = " + ContactObj.getClass());
	
		contactType = ContactObj.getPeople().getContactType();	logDebug("contactType : " + contactType);
		contactEmail = ContactObj.getPeople().getEmail();		logDebug("contactEmail : " + contactEmail);
		contactRelation = ContactObj.getPeople().getRelation();	logDebug("contactRelation : " + contactRelation);
		contactName = ContactObj.getPeople().getFullName() ;	logDebug("contactName : " + contactName);
		contactFirstName = ContactObj.getPeople().getFirstName() ;	logDebug("contactFirstName : " + contactFirstName);
		contactLastName = ContactObj.getPeople().getLastName() ;	logDebug("contactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);

		var attributes = peopleModel.getAttributes();

		ContactAttr = new Array();	
		if (attributes != null)
		{
			var attIter = attributes.iterator();

			while(attIter.hasNext())
    			{
				var contactAttributeModel = attIter.next();
    			ContactAttr[contactAttributeModel.attributeName] = contactAttributeModel.attributeValue;
    			logDebug("ContactAttr[\"" + contactAttributeModel.attributeName + "\"] = " + contactAttributeModel.attributeValue);
				}
		}
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	if (arguments.length == 6) // License Number provided
		...
		{
		var capLicensgDebug("Condition already exists: New condition not added to Address ID "+addrRefId);
...
function appMatch(ats) // optional capId or CapID string
	...
	var isMatch = true;
	var ata = ats.split("/");
	if (ata.ntactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
		...
		for (zz in Parcels)
			...
			for (a1 in fGisObj) // for each GIS object on the Cap
				...
				var gisTypeScriptModel = fGisObj[a1];
					...
					var retval = aa.gis.addCapGISObject(capId,gisObjModel.getServiceID(),gisObjModel.getLayerId(),gisObjMntactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
	...
	for (i in wfObj)
		{
  		...
  		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			...
			var fTaskModel = wfObj[i].getTaskItentactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
...
function getRefLicenseProf(refstlic)
	...
	if (!refLicenseResult.getSuccess())
		...
		for (var thisLic in newLicArray)
			if (refstlic && refstlic.toUpperCase().equals(newLicArray[thisLic].getStntactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	if (arguments.length == 6) // License Number provided
		...
		{
		var capLicens=pfObj[ij].getFeeAllocation()
				}
...
function lookupDateRange(stdChoiceEntry,dateValue) // optional val number
	...
	for (var count=1; count <= 9999; count++)  // Must be sequential from 01 up to 9999
		...
		var bizDomScripntactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice,pDuplicate,pFeeSeq)
    ...
    if ( pDuplicate==null || pDuplicate.length==0 )
        ...
        pDuplicate = pDuplicatentactLastName : " + contactLastName);
		contactBusiness = ContactObj.getPeople().getBusinessName();	logDebug("contactBusiness : " + contactBusiness);
