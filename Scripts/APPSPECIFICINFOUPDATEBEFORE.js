...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	for (var refLic in refLicArr)
		...
		if (addCAEResult.getSuccess())
			{
			lsfully added condition to Address ID " + addrRefId + "  (" + pImpact + ") " + pDesc);
					condAdded=true;
					}
				else
					{
...
// Bypasses current CAP
	...
	if (getCapResult.getSuccess())
		...
		{ logDebug( "**ERROR: getting caps by appiceResult.getErrorMessage());
	}
...
function copyFees(sourceCapId, targetCapId, activityID, period) {
	...
	feeQuantity = (period * thisFee.unit)
        ...
        if (forceInvoice && forceInvoiceYes) {
            addFeeWithExtraData(thisFee.code, thisFee.sched, thisFee.period, feeQuantity, "Y", targetCapId, aiceResult.getErrorMessage());
	}
...
function addLicenseCondition(cType,cStatus,cDesc,cComment,cImpact)
	...
	for (var refLic in refLicArr)
		...
		if (addCAEResult.getSuccess())
			{
			ltepnumber, "N", completeFlag, null, null)
...
function editRefLicProfAttribute(pLicNum,pAttributeName,pNewAttributeValue)
	...
	if (attrfound)
		...
		{
		logDebug("**WARNING attribute: " + pAttributeName + " not foiceResult.getErrorMessage());
	}
...
function loadASITablesBefore(gm)
	{
 	//
 	// Loads App Specific tables into their own array of arrays.  Creates global array objects
	//
	// Parameter is the before AppSpecificTableGroupModel
	//
	...
	while (tai.hasNext())
	  ...
	  tn = "UPDATED"+String(tn).replace(/[^a-zA-Z0-9]+/g,'');
...
function loadTasks(ltcapidstr)
	...
	if (typeof(ltcapidstr) == "string")
                {
		var ltresult = aa.cap.getCapID(ltcapidstr); 
	 	if (ltresult.iceResult.getErrorMessage());
	}
...
function taskStatusDate(wfstr) // optional process name, capId
    ...
    var itemCap = capId;
	if (arguments.length == 3) itemCapiceResult.getErrorMessage());
	}
