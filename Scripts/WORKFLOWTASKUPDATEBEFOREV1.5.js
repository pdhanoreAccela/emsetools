...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP inCondition.getAddressConditions(addrRefId);
...
function appHasCondition(pType,pStatus,pDesc,pImpact)
	...
	for (cc in capConds)
		...
		if ( (pStatus==null || pStatus.to "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP ingetErrorMessage());
			return false;
			}
		}
	}
...
function copyParcelGisObjects()
	...
	if (capParcelResult.getSuccess())
		...
		for (zz in Parcels)
			{
			var ParcelValidatedNumber = Parcels "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP ins.length == 3) itemCap = arguments[2]; // use cap ID specified in args
...
function editReportedChannel(reportedChannel) // option CapId
{
	...
	if (cdWrite.getSuccess( "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP in aa.print("**ERROR: Getting GIS Type for Buffer Target.  Reason is: " + bufferTargetResult.getErrorType() + ":" + bufferTargetResult.getErrorMessage()) ; return false }
...
function getParents(pAppType)
	{
	 ...
	 "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP in		 	itemCap = ltresult.getOutput();
	  			else
...
function lookup(stdChoice,stdValue)
	{
	var strControl;
	var bizDomScriptResult = aa.bizDomain.getBizDomainByValue(stdChoice,stdVal "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP inay.length)
		{
			for(x in parentArray)
...
function updateAppStatus(stat,cmt) // optional cap id
	...
	if (updateStatusResult.getSuccess())
		...
		logDebug("**ERROR: application status update to " + stat + " was unsuccessful.  The reaso "." + wfTask + "." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		else
			AInfo["Updated." + wfTSI[TSIm].getCheckboxDesc()] = wfTSI[TSIm].getChecklistComment();
		}
	}
...
function addFee(fcode,fsched,fperiod,fqty,finvoice) // Adds a single fee, optional argument: fCap
	...
	if (assessFeeResult.getSuccess())
		...
		if (finvoice == "Y" && arguments.length > 5) // use CAP in