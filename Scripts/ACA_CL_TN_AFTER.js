/*------------------------------------------------------------------------------------------------------/
| Program : ACA_CL_TN_After_V1.0.js
| Event   : ACA_CL_TN_After
|
...
var controlString = "ACA CL PageFlowScript After TN"; 			// Standard choice for control
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (!apdResult.getSuccess())
			{ logDebug("**ERROR Adding District " + districtValue + " to parccapCondition.getCapConditions(capId);
	else
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (!apdResult.getSuccess())
			{ logDebug("**ERROR Adding District " + districtValue + " to parc = capId;
	else
...
function createRefLicProf(rlpId,rlpType,pContactType)
	...
	if (AInfo["Insurance Co"]) 		newLic.setInsuranceCo(AInfo["I;
	else
		mth = "0"+pMonth.toString();
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (!apdResult.getSuccess())
			{ logDebug("**ERROR Adding District " + districtValue + " to parcEndDate.getDate()+"/"+jsEndDate.getFullYear());
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (!apdResult.getSuccess())
			{ logDebug("**ERROR Adding District " + districtValue + " to parc
...
//if parcelNum is null, district is is added to all parcels on CAP
	...
	if (!parcelNum)
		...
		if (!apdResult.getSuccess())
			{ logDebug("**ERROR Adding District " + districtValue + " to parc= fTask.getStepNumber();
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			{
                        ...
                        assignDate.setTime(tempDate.getTim;
	else
		mth = "0"+pMonth.toString();
