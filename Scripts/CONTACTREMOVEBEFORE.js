/*------------------------------------------------------------------------------------------------------/
| SVN $Id: ContactRemoveBefore.js 4687 2009-08-21 21:35:56Z john.schomp $
| Program : ContactRemoveBeforeV1.6.js
...
var capId = aa.env.getValue("CapId");							// CapId object
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParceletAddressByCapId(capId);
	if (capAddResult.getSuccess())
...
function assignCap(assignId) // option CapId
	...
	iObj = iObjResuMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParcel = aa.address.getAddressByCapId(vToCapId);
...
{
	var ownrReq = aa.owner.getOwnerByCapId(sCapIMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParcel
    ...
    for (xx in pairObjArray) {
        ...
        if (doExecution) {
            if (doObj.enabled) {
               ...
               Message) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
...
function getChildren(pCapType, pParentCapId)
	...
	for (xx in childArray)
		{
		childCapId = childArray[xx].getCapID(Message) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParcelfObj[i];
	...
	for (i in wfObj)
		{
 		...
 		if (fTask.getTaskDescription().toUpperCase().equals(wfstr.toUpperCase())  && (!useProcess || fTask.getProcessCode().equals(processName)))
			if (fTask.getCompleteFlag().equals("Y"))
				...
				retMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParcele(domArr[0])
...
function proximity(svc,layer,numDistance)  // optional: distanceType
	...
	// use with all eMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
		...
		if (capParcelResult.getSuccess())
			...
			for (zz in Parcels)
				...
				if (!apdResult.getSuccess())
					...
					logDebug("Successfully added district " + districtValue + " to parcel #" + Parcels[zz].getParcelssName = "";
	var itemCap = capId;
	if (arguments.length >= 2)
		{
		processName = arguments[1]; // subprocess
        ...
        for (x in capPrclArr)
        {
	        ...
	        if (prclObj.getSuccess() )
	        {
		        ...
		        if (prclArr.length)
		        {
			        ...
			        elMessage) aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + message);
		if (showDebug) 	aa.env.setValue("ScriptReturnMessage", "<font color=red><b>Action Cancelled</b></font><br><br>" + debug);
		}
	else
		{
