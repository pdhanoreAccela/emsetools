...
function addStdCondition(cType,cDesc) // optional cap ID
    ...
    if (!aa.capCondition.getStandardConditions)
        ...
        for(i = 0; i<standardConditions.length;i++)
            ...
            if (addCapCondResult.getSuccess())
                {
                logDebug("Successfully added condition (" + standardCondition.getConditionDesc() + ")" + " to record " + itemCap.getCustomID());
                }
            else
                {
...
function externalLP_CA(licNum,rlpType,doPopulateRef,doPopulateTrx,itemCap)
    ...
    */
    ...
    for (var thisLic = 0; thisLic < workArray.length; thisLic++)
        ...
        aURLArgList[0] = "https://aa.achievo.com/License+Detail.asp?LicNum=" + licNum;
        var oURL = aa.proxyInvoker.newInstance("java.net.URL",aURLArgList).getOutput();
        var document = saxBuilder.build(oURL); //("https://aa.achievo.com/License+Detail.asp?LicNum=" + licNum);
...
function loadFees()  // option CapId
    ...
    var feeResult=aa.fee.getFeeItems(itemCap);
        ...
        for (ff in feeObjArr)
            ...
            // var xInvoiceModel = aa.finance.getFeeItemInvoiceByFeeNbr(itemCap,fFee.getFeeSeqNbr(),null).getOutput();
            // if(xInvoiceModel !=null){
                // for(xi in xInvoiceModel) aa.print("Method: " + xInvoiceModel[xi]);
            // }
            // else
            // {
                // aa.print("xInvoiceModel is null");
            // }
            
            myFee.sequence = fFee.getFeeSeqNbr();
            myFee.code =  fFee.getFeeCod();
            //myFee.priority = fFee.getPriority();
            ...
            //myFee.invoice = fFee.getInvoiceNbr();
...
function Fee() // Fee Object
    ...
    this.invoice = null; // getInvoiceNbr()
    }
...
function loadContactASI(thisArr)
{
    contactASI = new Array();
    var cons = aa.people.getCapContactByCapID(capId); 
    if(cons.getSuccess()) 
    { 
        cons = cons.getOutput(); 
        for (x in cons) 
        { 
            ...
            var people = cons[x].getPeople(); 
            if(null != people) 
            { 
                var contactType = people.getContactType();
                //aa.print(conSeqNum + ": " + contactType);
                var genericTemplate = people.getTemplate(); 
                if(null != genericTemplate) 
                { 
                    var templateForms = genericTemplate.getTemplateForms(); 
                    if(null != templateForms) 
                    { 
                        for(var y = 0; y < templateForms.size();y++) 
                        { 
                            var subgroups = templateForms.get(y).getSubgroups(); 
                            if(null != subgroups) 
                            { 
                                for(var j = 0; j < subgroups.size();j++) 
                                { 
                                    var fields = subgroups.get(j).getFields(); 
                                    var rows = subgroups.get(j).getRows(); 
                                    if(null != fields) 
                                    { 
                                        for(var k = 0; k< fields.size();k++) 
                                        { 
                                            var displayFieldName = fields.get(k).getDisplayFieldName(); 
                                            var defaultValue = fields.get(k).getDefaultValue(); 
                                            //aa.print(" displayFieldName ---" + displayFieldName); 
                                            //aa.print("field value :" + defaultValue); 
                                            thisArr[fields.get(k).getDisplayFieldName()] = fields.get(k).getDefaultValue();
                                        } 
                                    } 
                                } 
                            } 
                    ...
                    /*for (a in thisArr)
                        aa.print("{" + a + "} = " + thisArr[a]);*/
            }
        }
    } 
    else 
    { 
    aa.print('No contact linked to the record!'); 
    }
}